...
export class ExtensionBackedSessionsProvider extends Disposable implements ISessionsProvider {
	...
	private async _refreshBrowseActions(): Promise<void> {
		try {
			const actionsResult = await this._commandService.executeCommand(
				SESSION_BRIDGE_COMMANDS.getBrowseActions,
				{}
			);
			if (!Array.isArray(actionsResult) || actionsResult.length === 0) {
				this._browseActions = [{
			...
			const actions = actionsResult as BrowseActionInfo[];
	...
	setModel(sessionId: string, modelId: string): void {
		...
		if (session) {
			...
			(session.mainChat.modelId as ReturnType<typeof observableValue<string>>).set(modelId, undefined);
		}
	}
	...
	private async _sendToExtensionSession(session: ISession, options: ISendRequestOptions): Promise<void> {
		...
		const selectedModelId = session.modelId.get() ?? session.mainChat.modelId.get() ?? this._defaultModelId;
